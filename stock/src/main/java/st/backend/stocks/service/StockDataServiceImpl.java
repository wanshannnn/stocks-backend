...
public class StockDataServiceImpl implements StockDataService {
    ...
    @Override
    public List<StockDataDTO> getAllStockDataById(String id) {
        Query query = new Query();
    ...
    @Override
    public List<StockDataDTO> getAllStockDataByName(String name) {
        Query query = new Query();
        ...
        List<StockDataDTO> dataDTOs = results.stream()
                .map(StockDataConverter::converterStockData)
                .collect(Collectors.toList());
        logger.info("已查找到name: {} 的全部数据", name);
    ...
    @Cacheable(
            value = "dataCache",
            key = "'stock_' + #id + '_data_' + #pageable.pageNumber + '_' + #pageable.pageSize")
    public Page<StockDataDTO> getAllStockDataByIdAndPage(String id, Pageable pageable) {
        Query query = new Query()
                .addCriteria(Criteria.where("id").is(id))
        ...
        List<StockDataDTO> dataDTOs = results.stream()
                .map(StockDataConverter::converterStockData)
                .collect(Collectors.toList());
        logger.info("已查找到id: {} 的全部数据，并按页返回", id);
    ...
    @Cacheable(
            value = "dataCache",
            key = "'stock_' + #name + '_data_' + #pageable.pageNumber + '_' + #pageable.pageSize")
    public Page<StockDataDTO> getAllStockDataByNameAndPage(String name, Pageable pageable) {
        Query query = new Query()
                .addCriteria(Criteria.where("name").is(name))
                .with(Sort.by("beijingTime").ascending())
                .with(pageable);
        ...
        List<StockDataDTO> dataDTOs = results.stream()
                .map(StockDataConverter::converterStockData)
                .collect(Collectors.toList());
        logger.info("已查找到name: {} 的全部数据，并按页返回", name);
